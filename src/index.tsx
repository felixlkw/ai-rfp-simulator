    ...
    <body class="bg-gray-100 min-h-screen">
        ...
        <div class="container mx-auto px-4 py-8">
            ...
            <div class="text-center space-x-4">
                ...
                <button onclick="startDemo()" class="bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-lg text-lg font-medium transition-colors">
                    <i class="fas fa-rocket mr-2"></i>데모 체험하기
                </button>
            </div>

            <!-- 데모 진행 상황 표시 -->
            <div id="demo-progress" class="hidden mt-8 bg-white rounded-lg shadow-sm p-6">
                <h3 class="text-xl font-semibold text-gray-900 mb-4">
                    <i class="fas fa-rocket text-green-600 mr-2"></i>데모 진행 상황
                </h3>
                <div class="space-y-4">
                    <div id="demo-step-1" class="flex items-center p-3 rounded-lg border transition-colors">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-gray-300 text-white">
                            <i class="fas fa-search text-sm"></i>
                        </div>
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900">1단계: 딥리서치 수집</h4>
                            <p class="text-sm text-gray-600">금고석유화학 15속성 데이터 수집</p>
                        </div>
                        <div id="demo-status-1" class="text-gray-400">대기 중</div>
                    </div>
                    
                    <div id="demo-step-2" class="flex items-center p-3 rounded-lg border transition-colors">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-gray-300 text-white">
                            <i class="fas fa-file-alt text-sm"></i>
                        </div>
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900">2단계: RFP 분석</h4>
                            <p class="text-sm text-gray-600">DX 프로젝트 RFP 15속성 분석</p>
                        </div>
                        <div id="demo-status-2" class="text-gray-400">대기 중</div>
                    </div>

                    <div id="demo-step-3" class="flex items-center p-3 rounded-lg border transition-colors">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-gray-300 text-white">
                            <i class="fas fa-user-plus text-sm"></i>
                        </div>
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900">3단계: AI 가상고객 생성</h4>
                            <p class="text-sm text-gray-600">30속성 통합 페르소나 생성</p>
                        </div>
                        <div id="demo-status-3" class="text-gray-400">대기 중</div>
                    </div>

                    <div id="demo-step-4" class="flex items-center p-3 rounded-lg border transition-colors">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-gray-300 text-white">
                            <i class="fas fa-clipboard-check text-sm"></i>
                        </div>
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900">4단계: 제안서 평가</h4>
                            <p class="text-sm text-gray-600">PwC 컨설팅 제안서 6지표 평가</p>
                        </div>
                        <div id="demo-status-4" class="text-gray-400">대기 중</div>
                    </div>

                    <div id="demo-step-5" class="flex items-center p-3 rounded-lg border transition-colors">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-gray-300 text-white">
                            <i class="fas fa-microphone text-sm"></i>
                        </div>
                        <div class="flex-1">
                            <h4 class="font-medium text-gray-900">5단계: 발표 평가</h4>
                            <p class="text-sm text-gray-600">발표 음성 분석 및 6지표 평가</p>
                        </div>
                        <div id="demo-status-5" class="text-gray-400">대기 중</div>
                    </div>
                </div>

                <!-- 데모 결과 요약 -->
                <div id="demo-results" class="hidden mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                    <h4 class="font-semibold text-green-900 mb-2">
                        <i class="fas fa-check-circle mr-2"></i>데모 완료!
                    </h4>
                    <p class="text-green-800 mb-3">금고석유화학 DX 프로젝트 평가가 성공적으로 완료되었습니다.</p>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="font-medium text-green-900">제안서 점수:</span>
                            <span id="proposal-score" class="text-green-800">4.17/5.0</span>
                        </div>
                        <div>
                            <span class="font-medium text-green-900">발표 점수:</span>
                            <span id="presentation-score" class="text-green-800">4.17/5.0</span>
                        </div>
                    </div>
                    <button onclick="viewDemoResults()" class="mt-3 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-sm transition-colors">
                        상세 결과 보기
                    </button>
                </div>
            </div>
        ...
        <script>
            ...
            let demoProposalEvalId = null;
            let demoPresentationEvalId = null;
            ...
            async function startDemo() {
                ...
                try {
                    // 1단계: 딥리서치 수집
                    await simulateStep(1, '딥리서치 수집 중...', async () => {
                        const response = await axios.get('/api/demo/deep-research');
                        return response.data.success;
                    });

                    // 2단계: RFP 분석  
                    await simulateStep(2, 'RFP 분석 중...', async () => {
                        const response = await axios.get('/api/demo/rfp-analysis');
                        return response.data.success;
                    });

                    // 3단계: AI 가상고객 생성
                    await simulateStep(3, 'AI 가상고객 생성 중...', async () => {
                        const response = await axios.post('/api/demo/generate-customer');
                        if (response.data.success) {
                            demoCustomerId = response.data.data.id;
                            return true;
                        }
                        return false;
                    });
                    ...
                    // 5단계: 발표 평가
                    await simulateStep(5, '발표 평가 중...', async () => {
                        const response = await axios.post('/api/demo/evaluate-presentation', {
                            customer_id: demoCustomerId  
                        });
                        if (response.data.success) {
                            demoPresentationEvalId = response.data.data.id;
                            return true;
                        }
                        return false;
                    });
                    ...
                    alert('데모 실행 중 오류가 발생했습니다. 다시 시도해주세요.');
                }
            }

            // 단계별 시뮬레이션 함수
            async function simulateStep(stepNumber, loadingMessage, apiCall) {
                const step = document.getElementById(\`demo-step-\${stepNumber}\`);
                const status = document.getElementById(\`demo-status-\${stepNumber}\`);
                ...
                try {
                    ...
                    if (success) {
                        ...
                        step.classList.add('border-green-500', 'bg-green-50');
                    ...
                    // 실패 상태
                    step.classList.remove('border-blue-500', 'bg-blue-50');
                    step.classList.add('border-red-500', 'bg-red-50');
                    icon.classList.remove('bg-blue-500');
            ...
            function resetDemoSteps() {
                for (let i = 1; i <= 5; i++) {
                    ...
                    step.className = 'flex items-center p-3 rounded-lg border transition-colors';
                    icon.className = 'w-8 h-8 rounded-full flex items-center justify-center mr-3 bg-gray-300 text-white';
            ...
            function showDemoResults() {
                ...
                const proposalScore = document.getElementById('proposal-score');
                const presentationScore = document.getElementById('presentation-score');
                
                proposalScore.textContent = '4.17/5.0';
                presentationScore.textContent = '4.17/5.0';
                
                results.classList.remove('hidden');
            }

            // 데모 결과 상세 보기
            function viewDemoResults() {
                if (demoCustomerId) {
                    // 실제 구현에서는 통합 결과 페이지로 이동
                    alert(\`데모가 성공적으로 완료되었습니다!\\n\\n생성된 AI 가상고객 ID: \${demoCustomerId}\\n제안서 평가 ID: \${demoProposalEvalId}\\n발표 평가 ID: \${demoPresentationEvalId}\\n\\n※ 통합 결과 페이지는 구현 예정입니다.\`);
                }
            }
        </script>
    </body>
